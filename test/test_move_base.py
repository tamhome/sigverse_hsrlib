...
class HSRBMoveIt(Logger):
    ...
    def broadcast_tf(self, position, orientation, target_frame, base_frame="odom",):
    ...
    def move_to_tf_target(self, target_frame, base_frame="odom"):
        try:
            # リスナーを使用して、base_linkからtarget_frameへの変換を取得します
    ...
    def move_group_python_interface(self):
        ...
        return self.all_close(pose_target, current_pose, 0.01)
    ...
    def run(self):
        ...
        # position = [0.2, 1.0, 0.0]  # 位置: x, y, z
        # orientation = [0.0, 0.0, 0.0, 1.0]  # 姿勢: 四元数 qx, qy, qz, qw
        # self.broadcast_tf(position, orientation, target_frame)

        # エンドエフェクタを指定したTFフレームに移動
        success = self.move_to_tf_target(target_frame)
        if success:
